...
test("keeps legacy event colors and fills the template background", () => {
  ...
  assert.deepEqual(normalizeTheme({ primaryColor: "#111111", accentColor: "#ff00aa", fontStyle: "princesa" }, fallback), {
    primaryColor: "#111111", accentColor: "#ff00aa", backgroundColor: fallback.backgroundColor, titleColor: fallback.titleColor, fontStyle: "princesa",
  });
});

test("keeps an organizer-selected background color", () => {
  const theme = normalizeTheme({ primaryColor: "#19324a", accentColor: "#e4a12c", backgroundColor: "#dcecf5", titleColor: "#112233", fontStyle: "refinada" });
  assert.equal(theme.backgroundColor, "#dcecf5");
  assert.equal(theme.titleColor, "#112233");
});
