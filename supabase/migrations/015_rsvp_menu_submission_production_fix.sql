-- Production-safe repair: preserves all existing events, guests, responses and invitation tokens.
...
begin
  ...
  if v_count>0 and (jsonb_typeof(coalesce(p_member_food_preferences,'[]'::jsonb))<>'array' or (select count(*) from jsonb_array_elements(p_member_food_preferences))<>v_count or exists(select 1 from jsonb_array_elements(p_member_food_preferences) preference where preference->>'memberId' is null or preference->>'foodPreference' not in ('Tradicional','Vegetariana','Vegana')) or exists(select 1 from jsonb_array_elements(p_member_food_preferences) preference where not exists(select 1 from public.guest_members member where member.guest_group_id=p_group_id and member.id=(preference->>'memberId')::uuid and member.id=any(p_attendee_ids)))) then raise exception 'member food preference required'; end if;
  ...
  insert into public.guest_member_food_preferences(guest_member_id,food_preference) select (preference->>'memberId')::uuid, preference->>'foodPreference' from jsonb_array_elements(coalesce(p_member_food_preferences,'[]'::jsonb)) preference;
