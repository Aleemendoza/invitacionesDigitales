...
export default async function Page({params}:{params:Promise<{slug:string}>}){const{slug}=await params;const template=templates.find((item)=>item.slug===slug);if(!template)notFound();const plan=planDetails[template.plan];return <main><AnalyticsView event={{name:"template_view",templateSlug:template.slug,plan:template.plan,eventType:template.category}}/><SiteHeader/><section className="preview"><div><p className="eyebrow">{template.category} · {plan.name}</p><h1>{template.name}</h1><p className="lead">Una experiencia {template.style.toLowerCase()} preparada para que tu celebración se sienta desde el primer mensaje.</p><ul className="previewFeatures"><li>Agenda, ubicación y detalles en un solo link</li><li>Vista optimizada para celular</li><li>{template.plan==="standard"?"Sin RSVP en este plan":template.plan==="premium"?"RSVP integrado y galería":"RSVP individual, álbum QR y trivia"}</li></ul><AnalyticsLink className="button pink" href={`/crear?template=${template.slug}`} analytics={{name:"create_start",templateSlug:template.slug,plan:template.plan,eventType:template.category}}>Usar esta plantilla</AnalyticsLink></div><TemplateCard template={template}/></section><SiteFooter/></main>}
