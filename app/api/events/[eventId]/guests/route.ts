...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ eventId: string }> }) {
  try { const result = await context(request, params); if ("error" in result) return result.error; if (!allowed(result.event)) return NextResponse.json({ error: "La lista de invitados es exclusiva de Premium Plus+." }, { status: 403 }); const guestId = new URL(request.url).searchParams.get("guestId"); if (!guestId) return NextResponse.json({ error: "Invitado inválido." }, { status: 400 }); const { error } = await result.db.from("guest_groups").delete().eq("id", guestId).eq("event_id", result.event.id); if (error) throw error; return NextResponse.json({ ok: true }); }
