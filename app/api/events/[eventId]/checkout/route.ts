...
import { isPlan, normalizePlan, planDetails } from "@/lib/event-drafts";
...
export async function POST(request:NextRequest,{params}:{params:Promise<{eventId:string}>}){try{
  ...
  const plan=isPlan(body.plan)?body.plan:normalizePlan(event.plan),idempotencyKey=typeof body.idempotencyKey==="string"&&/^[a-zA-Z0-9:_-]{16,128}$/.test(body.idempotencyKey)?body.idempotencyKey:`initial:${event.id}:${plan}`;
