...
export async function POST(request:NextRequest,{params}:{params:Promise<{eventId:string}>}){try{
  ...
  const body=await request.json() as{targetPlan?:unknown;idempotencyKey?:unknown};if(!isPlan(body.targetPlan)||planRank(body.targetPlan)<=planRank(context.event.plan as Plan))return NextResponse.json({error:"Elegí un plan superior."},{status:400});
  const amount=planDetails[body.targetPlan].price-planDetails[context.event.plan as Plan].price;if(amount<=0)return NextResponse.json({error:"El plan ya está activo."},{status:409});
