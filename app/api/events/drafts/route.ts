...
export async function POST(request: NextRequest) { try {
  ...
  const { data, error } = await db.from("events").insert({ owner_id: user.id, slug, title: input.title.trim(), event_type: input.eventType, starts_at: startsAt(input.date, input.time), template_slug: input.templateSlug, plan: input.plan, payment_status: "unpaid", content: { venue: input.venue, venueAddress: input.venueAddress ?? "", mapUrl: input.mapUrl ?? "", closingMessage: input.closingMessage ?? "", organizerWhatsapp: input.organizerWhatsapp ?? "", wizard_step: 7, features: input.features.length ? input.features : defaultFeatures(input.plan), agenda: input.agenda, message: input.message ?? "", dressCode: input.dressCode ?? "", musicUrl: input.musicUrl ?? "", theme: input.theme, rsvp: input.rsvp } }).select("id,slug,plan,payment_status").single();
