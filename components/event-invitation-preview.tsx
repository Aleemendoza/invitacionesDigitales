...
export type InvitationPreviewData = Pick<StoredEvent, "title" | "event_type" | "starts_at" | "template_slug"> & { content: EventContent; photos?: string[]; sections?: EventSections; welcomeBackgroundUrl?: string };
...
function Countdown({ startsAt, image, preview }: { startsAt: string | null; image: string; preview: boolean }) {
  ...
  if (state.kind !== "pending") return <section className="eventCountdown" style={{ "--countdown-image": `url(${image})` } as CSSProperties}><p>{state.kind === "missing" ? "Cuenta regresiva" : "Hoy es el día"}</p><strong>{state.kind === "missing" ? <>Elegí fecha y hora<br />para activarla.</> : <>¡Llegó el momento<br />de celebrar!</>}</strong></section>;
  return <section className="eventCountdown" style={{ "--countdown-image": `url(${image})` } as CSSProperties}><p>Falta muy poco</p><div>{[[state.days, "Días"], [state.hours, "Hs"], [state.minutes, "Min"], [state.seconds, "Seg"]].map(([value, label]) => <span key={String(label)}><b>{String(value).padStart(2, "0")}</b><small>{label}</small></span>)}</div></section>;
}
...
export function EventInvitationPreview({ event, label = "Vista previa", plan, onCheckout = () => window.location.assign("/precios"), onEdit = () => window.scrollTo({ top: 0, behavior: "smooth" }) }: { event: InvitationPreviewData; label?: string; plan?: Plan; onCheckout?: () => void; onEdit?: () => void }) {
  ...
  useEffect(() => { setShowWelcome(true); }, [event.title, event.event_type, event.content.welcome?.message, event.welcomeBackgroundUrl]);
  ...
  if (showWelcome) return <aside className="invitationPreview" style={style}><span>{label}</span><div className="phoneFrame"><div className="phoneScreen"><InvitationWelcome compact title={event.title} eventType={event.event_type} message={event.content.welcome?.message} backgroundUrl={welcomeBackground} accentColor={theme.accentColor} onStart={() => setShowWelcome(false)} /></div></div></aside>;
