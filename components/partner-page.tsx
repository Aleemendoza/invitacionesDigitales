...
const benefits: Benefit[] = [
  ...
  { icon: "gift", title: "Un beneficio para tus clientes", body: "Cuando alguien reserva tu espacio, podés ofrecerle Celebra para reunir cada detalle de su casamiento, cumpleaños, XV, bautismo u otra celebración." },
  ...
  { icon: "hand", title: "Nosotros nos ocupamos", body: "No necesitás diseñar invitaciones, brindar soporte ni aprender un sistema nuevo. Tu equipo recomienda Celebra; nosotros hacemos el resto." },
];
...
const steps = [
  ...
  ["03", "Recomendás Celebra", "Cuando reservan un evento, compartís un QR o enlace exclusivo."],
...
function PartnerHeader() {
  return <header className="partnerHeader"><Link className="brand" href="/">celebra<span>✦</span></Link><nav><Link href="/plantillas">Plantillas</Link><Link href="/precios">Precios</Link><Link href="/partner" aria-current="page">Celebra Partner</Link><Link className="button pink" href="/crear">Crear invitación</Link></nav></header>;
}

function PartnerFooter() {
  return <footer className="siteFooter"><div className="footerTop"><div><Link className="brand" href="/">celebra<span>✦</span></Link><p>La forma más linda de reunir a quienes más querés.</p></div><div className="footerLinks"><div><b>Descubrí</b><Link href="/plantillas">Plantillas</Link><Link href="/precios">Planes y precios</Link><Link href="/partner">Celebra Partner</Link></div><div><b>¿Necesitás ayuda?</b><a href="mailto:hola@celebra.app">hola@celebra.app</a><a href={partnerWhatsappUrl} target="_blank" rel="noreferrer">WhatsApp</a></div></div></div><div className="footerBottom"><span>© 2026 Celebra. Hecho para grandes momentos.</span></div></footer>;
}

export function PartnerPage() {
  return <main><PartnerHeader/>
    <section className="partnerHero"><div><p className="eyebrow">PROGRAMA CELEBRA PARTNER</p><h1>Hacé crecer tu espacio.<br/><em>Nosotros te ayudamos a digitalizarlo.</em></h1><p className="lead">Si tenés un salón de eventos, quinta, casa de alquiler o espacio para celebraciones, podés formar parte de Celebra Partner.</p><p className="partnerHeroCopy">Vos acercás Celebra a quienes organizan un evento en tu espacio. Nosotros te damos herramientas digitales para potenciar tu negocio.</p><p><a className="button pink" href={partnerWhatsappUrl} target="_blank" rel="noreferrer">Quiero ser Partner →</a> <a className="button partnerOutline" href="#como-funciona">Conocer cómo funciona</a></p><div className="partnerProof"><span>Sin inversión inicial</span><span>Sin sistemas complicados</span></div></div><aside className="partnerHeroVisual"><p className="eyebrow">TU ESPACIO, MÁS PRESENTE</p><Icon name="pin" size={34}/><strong>Una invitación que también muestra dónde sucede la celebración.</strong><span>Ubicación · Mapa · Información del espacio</span><div className="partnerQr">QR<small>Tu enlace Partner</small></div></aside></section>
    ...
    <section className="partnerExperience section"><div><p className="eyebrow">TU ESPACIO PUEDE TENER SU PROPIA EXPERIENCIA CELEBRA</p><h2>Más que una dirección.<br/><em>El comienzo de la celebración.</em></h2><p className="sectionLead">Una pareja que reserva su casamiento en tu salón puede enviar una invitación donde los invitados encuentren fecha, horarios, ubicación, mapa, confirmación, dress code, regalos, fotografías, música e información del evento.</p></div><div className="partnerPhone"><p>CASAMIENTO</p><h3>Sofía &amp; Mateo</h3><span>Sábado 12 de diciembre</span><hr/><strong>Estancia La Candelaria</strong><small>Ver ubicación en el mapa →</small></div></section>
