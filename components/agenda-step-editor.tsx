...
export function AgendaStepEditor({ agenda, onChange }: { agenda: AgendaItem[]; onChange: (agenda: AgendaItem[]) => void }) {
  ...
  return <fieldset className={styles.agenda}>
    ...
    <div className={styles.list}>
      {agenda.map((item, index) => <div className={styles.item} key={`${index}-${item.time}`}>
        ...
        <label>Hora
          <input aria-label={`Hora del momento ${index + 1}`} type="time" value={item.time} onChange={(event) => updateItem(index, "time", event.currentTarget.value)} />
        </label>
        <label>Momento
          <input aria-label={`Nombre del momento ${index + 1}`} placeholder="Ej.: Ceremonia" value={item.title} onChange={(event) => updateItem(index, "title", event.currentTarget.value)} />
        </label>
