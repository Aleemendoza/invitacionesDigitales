...
export function PublicInvitation({ event }: { event: StoredEvent & { event_sections?: Section[] } }) {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => setCountdown(getCountdown(event.starts_at, false)), 1000);
    return () => window.clearInterval(interval);
  }, [event.starts_at]);

  const theme = isTheme(event.content.theme)
    ? event.content.theme
    : { primaryColor: "#21191b", accentColor: "#e44f88", fontStyle: "clasica" as const };
  const style = {
  ...
  const date = event.starts_at
    ? new Intl.DateTimeFormat("es-AR", { day: "2-digit", month: "long", year: "numeric" }).format(new Date(event.starts_at))
    : "Fecha a confirmar";
  const gifts = event.event_sections?.find((section) => section.kind === "gifts");
  const social = event.event_sections?.find((section) => section.kind === "social_photos");
  const values = countdown.kind === "pending" ? [countdown.days, countdown.hours, countdown.minutes, countdown.seconds] : [0, 0, 0, 0];

  return (
    <main className={`publicInvite premiumInvite ${template.theme}`} style={style}>
      <section className="piHero">
        <div className="piHeroTools">
          <button aria-label="Abrir menú"><Icon name="menu" size={19} /></button>
          <button aria-label="Música"><Icon name="music" size={19} /></button>
        </div>
      ...
      <section className="piCountdownCard">
        ...
        <div>
          {values.map((value, index) => (
            <span key={index}><b>{format(value)}</b><small>{["Días", "Horas", "Minutos", "Segundos"][index]}</small></span>
          ))}
        </div>
      </section>

      <section className="piDetails">
        <Info icon="calendar" title="¿Cuándo?"><p>{date}<br />{event.starts_at && new Intl.DateTimeFormat("es-AR", { hour: "2-digit", minute: "2-digit" }).format(new Date(event.starts_at))} HS</p></Info>
        <Info icon="pin" title="¿Dónde?"><p>{event.content.venue}<br />{event.content.venueAddress}</p>{event.content.mapUrl && <a href={event.content.mapUrl} target="_blank" rel="noreferrer">Ver en mapa</a>}</Info>
      </section>
      ...
      <section className="piRsvp">
        <div className="piRsvpCopy"><Icon name="mail" /><div><p className="eyebrow">Asistencia</p><h2>Confirmá tu asistencia</h2><p>{event.content.rsvp?.deadline ? `Confirmá antes del ${new Intl.DateTimeFormat("es-AR", { day: "2-digit", month: "long", year: "numeric" }).format(new Date(event.content.rsvp.deadline))}` : "Tu respuesta es muy importante"}</p></div></div>
        {event.rsvp_enabled && <Link href={`/e/${event.slug}/rsvp`}>Confirmar</Link>}
      </section>
...
function SectionHeader({ icon, label }: { icon: "gallery"; label: string }) {
  return <header className="piSectionHeader"><Icon name={icon} /><p>{label}</p></header>;
}

function Info({ icon, title, children }: { icon: "calendar" | "pin"; title: string; children: React.ReactNode }) {
  return <article><Icon name={icon} /><h2>{title}</h2>{children}</article>;
}
