...
export function PublicInvitation({ event }: { event: StoredEvent & { event_sections?: Section[] } }) {
  ...
  if (!hasStarted) return <InvitationWelcome title={event.title} eventType={event.event_type} message={event.content.welcome?.message} backgroundUrl={welcomeBackground} accentColor={theme.accentColor} musicEmbedUrl={musicId ? youtubeEmbedUrl(musicId) : undefined} musicPlaying={musicPlaying} onToggleMusic={() => setMusicPlaying((current) => !current)} onStart={() => setHasStarted(true)} />;
