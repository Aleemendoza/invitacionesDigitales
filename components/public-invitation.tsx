...
export function InvitationExperience({ event, mode, initiallyStarted = false }: { event: StoredEvent; mode: InvitationRenderMode; initiallyStarted?: boolean }) {
  ...
  const [hasStarted, setHasStarted] = useState(initiallyStarted);
  ...
  return <div className={`invitationExperience${hasStarted ? "" : " hasWelcome"}`}><YouTubeMusicPlayer embedUrl={musicId ? youtubeEmbedUrl(musicId) : undefined} playing={musicPlaying} /><main className={`publicInvite premiumInvite ${template.theme}`} style={style} aria-hidden={!hasStarted}>
