...
const options = (type: string, plan: Plan) => (type === "Baby Shower" ? templates.filter((item) => item.slug === "dreamscape") : templates.filter((item) => item.category === category(type))).filter((item) => item.plan === plan);
...
export function CreateEventWizard() {
  ...
  const choosePlan = (plan: Plan) => { setPlanChosen(true); setDraft((current) => ({ ...current, plan, eventType: "", templateSlug: "", features: defaultFeatures(plan) })); };
  const chooseType = (eventType: string) => {
    const template = options(eventType, draft.plan)[0];
    if (!template) return;
  ...
  const selectedTemplate = templates.find((item) => item.slug === draft.templateSlug) ?? templates[0];
...
function Fields({ draft, photos, update, choosePlan, chooseType, chooseTemplate, setPhotos }: { draft: Draft; photos: Photo[]; update: <K extends keyof Draft>(key: K, value: Draft[K]) => void; choosePlan: (plan: Plan) => void; chooseType: (value: string) => void; chooseTemplate: (value: Template) => void; setPhotos: (value: Photo[]) => void }) {
  ...
  if (draft.step === 1) return <div className="choices">{types.filter((type) => options(type, draft.plan).length > 0).map((type) => <button className={draft.eventType === type ? "selected" : ""} onClick={() => chooseType(type)} key={type}>✦<br />{type}</button>)}</div>;
  ...
  if (draft.step === 6) return <div className="templatePicker">{options(draft.eventType, draft.plan).map((template) => <button className={draft.templateSlug === template.slug ? "selected" : ""} onClick={() => chooseTemplate(template)} key={template.slug}><span className="templateThumb" style={{ backgroundImage: `url(${template.coverImage})` }} /><b>{template.name}</b><small>{template.style}</small></button>)}</div>;
